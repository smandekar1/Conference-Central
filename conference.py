...
from models import SessionQueryDuration
from models import SessionQueryName
...
# --- Session Objects -------------     
    ...
    @endpoints.method(SessionQueryForm, SessionForms, path='sessionQuery',
    ...
    @endpoints.method(SessionQueryDuration, SessionForms, path='queryDate',
            http_method='GET', name='getSessionShorterThan')
    def getSessionShorterThan(self, request):
        """Get sessions that are shorter than number of minutes."""

        # query session, filter by duration  
        q = Session.query()
    ...
    @endpoints.method(SessionQueryName, SessionForms, path='queryName',
            http_method='GET', name='getSessionByName')
    def getSessionByName(self, request):
        """Get session by name."""

        # query session, filter by speaker 
        q = Session.query()
        q = q.filter(Session.name == request.name)
...
# TODO 1
    ...
    def _cacheFeaturedSpeaker(speaker):
        ...
        """
        featured = "Our current featured speaker is " + speaker
